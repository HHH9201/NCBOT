...
class Xydj(NcatBotPlugin):
    ...
    async def process_game_resource(self, game, event, force_update=False):
        ...
        try:
            ...
            # 从 .env 读取配置
            BACKEND_URL = os.getenv("BACKEND_URL", "http://127.0.0.1:8979")
    ...
    async def _wait_and_send_resource(self, event, group_id, ticket_id, game, english_keyword, chinese_display, user_id, user_nickname, force_update=False):
        ...
        # 告诉用户正在等待验证
        await event.reply(
            rtf=MessageArray([Reply(id=event.message_id), PlainText(text="✅ 请扫码在小程序内点击“点击获取”，完成后我将立即在此发送资源链接！")])
        )
        ...
        for i in range(max_retries):
            ...
            try:
                ...
                # 只有状态改变或出现异常时才打印日志，避免刷屏
                status = response.get("status") if isinstance(response, dict) else None
                if status and status != "pending":
                    print(f"[Poll Ticket] Ticket: {ticket_id}, Status: {status}")
